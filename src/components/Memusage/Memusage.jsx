...
export default class Memusage extends Component {
  ...
  displayMem(_redraw = false) {
    var data = [
      { inits: "Total", value: 10 },
      { inits: "Available", value: 100 },
      { inits: "Free", value: 60 }
    ];
    ...
    function generate(data, id) {
      var margin = {
        ...
        height = this.width - margin.top - margin.bottom;
        ...
        .on("mouseover", function(d) {
          ...
          d3.select(this)
            ...
            .attr(
              ...
              arc
                .innerRadius(innerRadius)
                .outerRadius((outerRadius / 0.75) * 0.9)
            );
    ...
    function redraw(data, path, arc) {
      ...
      path = path
        .datum(data)
        .data(donut)
        .attr("d", arc); // compute the new angles
      path
        .transition()
        .duration(750)
